...
class Tracker:
    def __init__(self, label, start_id, method):
        ...
        self.prev_box = np.array(self.label)[:,1:]      #前一帧
        self.curr_box = np.array(self.label)[:,1:]      #当前帧
        self.pprev_box = np.array(self.label)[:,1:]     #前前帧
...
def get_cost(label,speed,feature):
    ...
    t1 = time.time()
    matrix = creat_matrix(label,feature,fw,fh)      #matrix[i][j]表示第i个检测框，第j个特征点。matrix[i][j]值为10时，说明特征点j在检测框i内,否则为200
    ...
    for i in range(len(matrix)):    #for one bounding box
        ...
        index = np.array(np.where(matrix[i]==10)[0])        #记录哪几个特征点在检测框i中
        # print(f"[DEBUG] 框{i}的index长度: {len(index)}, index内容: {index}") 
